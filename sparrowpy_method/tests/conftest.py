...
def load_default_input_data():
    ...
    with open(os.path.join(
            default_data_path(),
            "test_input_sparrowpy.json"), 'r') as f:
        data = json.load(f)
...
def create_temporary_input_file():
    ...
    geo_file = os.path.join(
        default_data_path(), "test_room_sparrowpy.geo")
    msh_file = os.path.join(
        default_data_path(), "test_room_sparrowpy.msh")
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
        ...
        input_tmp['geo_path'] = os.path.join(
            tmpdirname, "test_room_sparrowpy.geo")
        input_tmp['msh_path'] = os.path.join(
            tmpdirname, "test_room_sparrowpy.msh")
...
def mock_requests_post():
    ...
    """
    with patch("sparrowpy_interface.definition.requests.post") as mock_post:
